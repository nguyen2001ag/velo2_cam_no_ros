...
//TODO: Change LIDAR config here
// TODO: Change this to a template if you can
namespace Velodyne {
struct Point {
...
std::vector<std::vector<Point *>> getRings(pcl::PointCloud<Velodyne::Point> &pc,
                                 int rings_count) {
  std::vector<std::vector<Point *>> rings(rings_count);
  for (pcl::PointCloud<Point>::iterator pt = pc.points.begin();
       pt < pc.points.end(); pt++) {
